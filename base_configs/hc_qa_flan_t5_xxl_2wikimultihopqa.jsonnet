...
local retriever_port = std.parseInt(std.extVar("RETRIEVER_PORT"));
local llm_server_host = std.extVar("LLM_SERVER_HOST");
local llm_server_port = std.parseInt(std.extVar("LLM_SERVER_PORT"));
local corpus_name = "2wikimultihopqa";
local dataset_name = "2wikimultihopqa";
local valid_qids = {
...

{
  ...
  "models": {
    ...
    "llmqa": {
      ...
      "lm_server_model_name": "google/flan-t5-xxl",
      "prompt_file": "prompts/2wikimultihopqa/nor.txt",
      "disable_qm": true,
      "next_model": "[EOQ]",
    },
  },
