#!/usr/bin/env python3
"""Add valid_qids and prompt_reader_args to HC config files"""

import re
import os
import glob

# Template for valid_qids and prompt_reader_args
VALID_QIDS_TEMPLATE = '''local valid_qids = {
...
def update_hc_config(filepath):
    """Update a single HC config file"""
    with open(filepath, 'r') as f:
        content = f.read()
    
    # Skip if already has valid_qids
    if 'local valid_qids' in content:
        print("Skipping {} - already has valid_qids".format(filepath.name))
        return False
    
    # Find the line after dataset_name definition
    pattern = r'(local dataset_name = "[^"]+";)\n'
    match = re.search(pattern, content)
    
    if not match:
        print("ERROR: Could not find dataset_name in {}".format(filepath.name))
        return False
    
    # Insert valid_qids and prompt_reader_args after dataset_name
    new_content = content[:match.end()] + '\n' + VALID_QIDS_TEMPLATE + '\n' + content[match.end():]
    
    # Update reader section to use prompt_reader_args
    new_content = re.sub(
        r'  "reader": \{',
        r'  "reader": prompt_reader_args + {',
        new_content
    )
    ...
    print("Updated {}".format(filepath.name))
    return True

def main():
    base_configs_dir = Path(__file__).parent / "base_configs"
    hc_configs = list(base_configs_dir.glob("hc_qa_*.jsonnet"))
    
    updated = 0
    for config_file in sorted(hc_configs):
        if update_hc_config(config_file):
            updated += 1
    
    print("\nUpdated {} out of {} HC config files".format(updated, len(hc_configs)))
